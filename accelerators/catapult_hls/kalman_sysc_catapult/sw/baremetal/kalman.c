...
#define DEV_NAME "sld,kalman_sysc_catapult"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_KALMAN, DEV_NAME);
	if (ndev == 0) {
		printf("kalman not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, KALMAN_MAC_N_REG, mac_n);
		iowrite32(dev, KALMAN_MAC_VEC_REG, mac_vec);
		iowrite32(dev, KALMAN_MAC_LEN_REG, mac_len);
