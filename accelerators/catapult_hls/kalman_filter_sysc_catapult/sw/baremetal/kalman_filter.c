...
#define SLD_KALMAN_FILTER 0x056
#define DEV_NAME "sld,kalman_filter_sysc_catapult"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_KALMAN_FILTER, DEV_NAME);
	if (ndev == 0) {
		printf("kalman_filter not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, KALMAN_FILTER_MAC_N_REG, mac_n);
		iowrite32(dev, KALMAN_FILTER_MAC_VEC_REG, mac_vec);
		iowrite32(dev, KALMAN_FILTER_MAC_LEN_REG, mac_len);
